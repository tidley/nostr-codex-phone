...
class WorkspaceCache {
  ...
  Future<void> _save({
    ...
    try {
      database.execute(
        '''INSERT INTO workspace_cache (cache_key, schema_version, snapshot_json, updated_at) VALUES (?, ?, ?, ?) ON CONFLICT(cache_key) DO UPDATE SET schema_version = excluded.schema_version, snapshot_json = excluded.snapshot_json, updated_at = excluded.updated_at''',
        [
          _cacheKey(localPubkey, servicePubkey),
          _schemaVersion,
  ...
  Future<Database> _open() async {
    ...
    database.execute(
      'CREATE TABLE IF NOT EXISTS workspace_cache (cache_key TEXT PRIMARY KEY, schema_version INTEGER NOT NULL, snapshot_json TEXT NOT NULL, updated_at INTEGER NOT NULL)',
    );
