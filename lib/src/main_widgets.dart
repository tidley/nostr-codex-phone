...
class _TeamWorkspace extends StatefulWidget {
  ...
  final ValueListenable<_WorkspaceFipsHeartbeat> fipsHeartbeat;
...
class _TeamWorkspaceState extends State<_TeamWorkspace> {
  ...
  static const _threadPaneMaxFraction = 0.9;
  ...
  final _threadSearch = _ThreadSearchController();
  ...
  final _threadCompletionOverrides = <String, bool>{};
  ...
  List<WorkspaceMessage> _cachedSearchMessages = const [];
  List<WorkspaceMessage> _cachedRootMessages = const [];
  Map<String, int> _cachedThreadReplyCounts = const {};
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final conversationKey = _conversationKey;
      if (mounted && conversationKey != null) {
        widget.onFocusConversation(conversationKey);
      }
    });
  }
  ...
  void _cacheActiveMessageProjection() {
    ...
    final conversationKey = _conversationKey;
    if (_cachedMessageRevision == revision &&
        _cachedMessageConversationKey == conversationKey) {
      return;
    }

    final visible = <WorkspaceMessage>[];
    final search = <WorkspaceMessage>[];
    final roots = <WorkspaceMessage>[];
    final repliesByParent = <String, List<WorkspaceMessage>>{};
    final replyCounts = <String, int>{};
    final lastReplyAt = <String, int>{};
    for (final message in _activeMessages) {
      final emptyAgentMessage = isWorkspaceEmptyAgentMessage(message);
      final hiddenThreadControlMessage =
      ...
      if (!emptyAgentMessage && !hiddenThreadControlMessage) {
        search.add(message);
      }
      if (message.parentId == null) {
        if (!emptyAgentMessage) visible.add(message);
        roots.add(message);
      } else {
        // Topic markers stay hidden from the transcript but are the source of
        // the topic tag shown on the root message bubble.
        repliesByParent.putIfAbsent(message.parentId!, () => []).add(message);
        if (!emptyAgentMessage && !hiddenThreadControlMessage) {
          if (message.alsoSendToMain) visible.add(message);
          replyCounts.update(
            message.parentId!,
            (count) => count + 1,
            ifAbsent: () => 1,
          ...
          lastReplyAt.update(
            message.parentId!,
            (createdAt) => math.max(createdAt, message.createdAt),
            ifAbsent: () => message.createdAt,
          );
        }
      }
    }
    ...
    for (final entry in repliesByParent.entries) {
      final topic = workspaceThreadTopic(entry.value);
      if (topic != null) topics[entry.key] = topic;
    }
    _cachedMessageRevision = revision;
    _cachedMessageConversationKey = conversationKey;
    _cachedVisibleMessages = visible;
    _cachedSearchMessages = search;
    _cachedRootMessages = roots;
    _cachedThreadReplyCounts = replyCounts;
    _cachedThreadLastReplyAt = lastReplyAt;
    _cachedThreadTopics = topics;
  }

  List<WorkspaceMessage> _threadMessagesFor(WorkspaceMessage thread) =>
      _activeMessages
          .where(
            (message) =>
  ...
  List<WorkspaceMessage> _threadRepliesFor(WorkspaceMessage thread) =>
      _threadMessagesFor(thread)
  ...
  void _requestThreadTopic(WorkspaceMessage thread, {bool automatic = false}) {
    ...
    final hasManualTopic =
        ...
        true;
    if (hasManualTopic ||
        (!automatic && (_threadTopicFor(thread) ?? 'Thread') != 'Thread') ||
        (automatic &&
            topicGeneration == 0 &&
            (_threadTopicFor(thread) ?? 'Thread') != 'Thread')) {
      return;
    }
  ...
  Future<void> _editActiveThreadTopic(BuildContext context) async {
    final thread = _thread;
    if (thread == null) return;
    final controller = TextEditingController(
      text:
          _threadTopicOverrideFor(thread.id).value ??
          _threadTopicFor(thread) ??
          'Thread',
    );
    ...
    _threadTopicOverrideFor(thread.id).value = cleaned;
    unawaited(
      widget.onRequest({
  ...
  List<WorkspaceMessage> get _openThreads => _openThreadIds
      .map(_threadWithId)
      .whereType<WorkspaceMessage>()
      .toList(growable: false);
  ...
  Map<String, String> _threadTopicsForActiveConversation() {
    ...
    final topics = Map<String, String>.of(_cachedThreadTopics);
    for (final message in _cachedRootMessages) {
      final override =
          _threadTopicOverrides['$_conversationKey:${message.id}']?.value;
      if (override?.trim().isNotEmpty == true) {
        topics[message.id] = override!.trim();
      }
    }
    return topics;
  }
  ...
  void _select(_WorkspaceSection section, String id) {
    ...
    setState(() {
      ...
      if (nextThread != null && !_openThreadIds.contains(nextThread.id)) {
        _openThreadIds.add(nextThread.id);
      }
      _expandedMessageIds
        ..clear()
        ..addAll(nextPanelState?.expandedMessageIds ?? const []);
      ...
      if (section == _WorkspaceSection.channel ||
          section == _WorkspaceSection.direct) {
        _recentConversations.removeWhere(
          (conversation) => conversation.$1 == section && conversation.$2 == id,
        );
  ...
  void _cycleConversation(int direction) {
    final conversations = <(_WorkspaceSection, String)>[
      for (final channel in widget.workspace.channels)
        if (widget.conversationPreferences[channel.id]?.archived != true)
          (_WorkspaceSection.channel, channel.id),
      for (final peer in widget.workspace.directPeers(widget.ownPubkey))
        if (widget
                ...
                ?.archived !=
            true)
          (_WorkspaceSection.direct, peer),
    ];
    if (conversations.isEmpty) return;
    final currentIndex = conversations.indexWhere(
      (conversation) =>
          conversation.$1 == _section && conversation.$2 == _active,
    );
    final nextIndex = currentIndex < 0
        ? 0
        : (currentIndex + direction + conversations.length) %
              conversations.length;
    final next = conversations[nextIndex];
    _select(next.$1, next.$2);
  }

  void _cycleRecentConversation(int direction) {
    if (_recentConversations.length < 2) return;
    final currentIndex = _recentConversations.indexWhere(
      (conversation) =>
          conversation.$1 == _section && conversation.$2 == _active,
    );
    final nextIndex = currentIndex < 0
        ? 0
        : (currentIndex + direction + _recentConversations.length) %
              _recentConversations.length;
    final next = _recentConversations[nextIndex];
    _select(next.$1, next.$2);
  }

  void _showThreads({String? topic}) {
    _closeDrawer();
    final previousKey = _conversationKey;
    if (previousKey != null) {
      _saveMainDraft();
  ...
  void _openSidebarThread(String conversationKey, String threadId) {
    if (_conversationKey != conversationKey &&
        widget.workspace.channels.any(
          (channel) => channel.id == conversationKey,
        )) {
      _select(_WorkspaceSection.channel, conversationKey);
    } else if (_conversationKey != conversationKey) {
  ...
  void _openReferencedMessage(WorkspaceMessage message) {
    if (message.channelId case final channelId?) {
      _select(_WorkspaceSection.channel, channelId);
    } else {
      final conversationKey = widget.workspace.conversationKeyForMessage(
        message,
      );
      final peer = widget.workspace
          .directPeers(widget.ownPubkey)
          ...
          .firstOrNull;
      if (peer == null) {
        _openAgentLastResponse(message);
        return;
      }
      _select(_WorkspaceSection.direct, peer);
    }
  ...
  void _showAgentDetails(WorkspaceMention mention) {
    final lastResponse = widget.workspace.messages.values
        .expand((messages) => messages)
        .where((message) => message.senderPubkey == 'agent:${mention.id}')
    ...
    showModalBottomSheet<void>(
      ...
      builder: (context) => _WorkspaceMentionDetails(
        ...
        onOpenLastResponse: lastResponse == null
            ...
            : () {
                Navigator.of(context).pop();
                _openAgentLastResponse(lastResponse);
              },
      ),
    );
  }
  ...
  void _closeCurrentThread() {
    if (_thread == null) return;
    final currentId = _thread!.id;
    setState(() {
      _saveThreadDraft();
      _openThreadIds.remove(currentId);
      _thread = _openThreads.lastOrNull;
      _alsoSendToMain = false;
      _threadFullWindow = false;
    ...
    final conversationKey = _conversationKey;
    final nextThread = _thread;
    if (conversationKey != null && nextThread != null) {
      widget.onOpenThread(conversationKey, nextThread.id);
    } else {
      widget.onCloseThread();
    }
  }
  ...
  void _setMessageExpanded(String messageId, bool expanded) {
    setState(() {
      if (expanded) {
  ...
  Widget build(BuildContext context) {
    ...
    final wide = medium;
    final sidebar = ValueListenableBuilder<_WorkspaceFipsHeartbeat>(
      valueListenable: widget.fipsHeartbeat,
      builder: (context, heartbeat, _) {
        final activeSpaceId = widget.activeSpace?.id;
        final fipsConnectedSpaceIds = {...widget.fipsConnectedSpaceIds}
          ..remove(activeSpaceId);
        if (heartbeat.connectionState == 'active' && activeSpaceId != null) {
          fipsConnectedSpaceIds.add(activeSpaceId);
        }
        return _WorkspaceSidebar(
          ...
          activityLabels: _conversationActivityLabels(),
          fipsConnected: heartbeat.connectionState == 'active',
          fipsConnectedSpaceIds: fipsConnectedSpaceIds,
          ...
          onToggleArchiveConversation: (section, id) {
            final key = _conversationKeyFor(section, id);
            if (key == null) return;
            final preference =
                widget.conversationPreferences[key] ??
    ...
    final Widget conversation = _section == _WorkspaceSection.threads
        ? _WorkspaceThreadsView(
            ...
            memberNames: widget.memberNames,
            memberAliases: widget.memberAliases,
            conversationDrafts: _conversationDrafts,
            threadDrafts: _threadDrafts,
            threadUnreadCounts: widget.threadUnreadCounts,
            onMarkAllRead: () {
              final conversationKeys = widget.threadUnreadCounts.keys
            ...
            builder: (context, _, _) {
              ...
              return _WorkspaceConversation(
                ...
                channelId: _section == _WorkspaceSection.channel
                    ? _active
                    : null,
                directPeer: _section == _WorkspaceSection.direct
                    ? _active
                    : null,
                ...
                threadCompletionOverrides: _threadCompletionOverrides,
                threadActivityLabels: _threadActivityLabels(typing),
                expandedMessageIds: _expandedMessageIds,
                onMessageExpandedChanged: _setMessageExpanded,
                composer: _composer,
                ...
                onOpenMessageReference: _openReferencedMessage,
                ...
                conversationFolderScope: widget.workspace
                ...
                onMentionSelected: (mention) => _insertMention(
                  _composer,
                  _selectedComposerMentions,
                  mention,
                ),
                typingStatuses: toastTyping,
                typingLabels: toastTyping
                    .map(_compactTypingLabel)
                    .toList(growable: false),
    ...
    final contextPane = ValueListenableBuilder<int>(
      ...
      builder: (context, _, _) {
        ...
        return _WorkspaceContext(
          ...
          relatedThreadCandidates: selectedThread == null
              ? const []
          ...
          threadSearch: _threadSearch,
          openThreads: _openThreads,
          threadTitles: {
            for (final thread in _openThreads)
              thread.id: _threadTopicFor(thread) ?? 'Thread',
          },
          threadUnreadCounts: {
            for (final entry in widget.threadUnreadCounts.entries)
              if (_conversationKey != null &&
          ...
          topicOverride: selectedThread == null
              ? _emptyThreadTopicOverride
              : _threadTopicOverrideFor(selectedThread.id),
          threadTopic: selectedThread == null
              ? null
          ...
          onReplyTargetOpened: () =>
              setState(() => _threadReplyTargetId = null),
        );
      },
    );
    ...
    final maxSidePaneWidth = (availableMessageWidth * _threadPaneMaxFraction)
        .clamp(_threadPaneMinWidth, availableMessageWidth);
    ...
    final threadUnreadCounts = {
      for (final entry in widget.threadUnreadCounts.entries)
        if (_conversationKey != null &&
            entry.key.startsWith('$_conversationKey:'))
          entry.key.substring(_conversationKey!.length + 1): entry.value,
    };
    final referencedThread = _thread == null
        ? null
        : _activeMessages
              .where(
                (message) =>
                    message.id ==
                    workspaceActiveRelatedThreadId(_activeMessages, _thread!),
              )
              .firstOrNull;
                ...
                ? AppBar(
                    ...
                    title: _openThreads.length < 2
                        ? Text(
                            _thread == null
                                ? 'Thread'
                                : _threadTopicFor(_thread!) ?? 'Thread',
                            maxLines: 1,
                            overflow: TextOverflow.ellipsis,
                        ...
                        : _ThreadTitleDropdown(
                            title: _thread == null
                                ? 'Thread'
                                : _threadTopicFor(_thread!) ?? 'Thread',
                            threads: _openThreads,
                            selectedThreadId: _thread!.id,
                            hasUnreadOtherThread: _openThreads.any(
                              (thread) =>
                    ...
                    actions: [
                      ...
                      PopupMenuButton<String>(
                        ...
                        onSelected: (action) {
                          ...
                          switch (action) {
                            case 'request-topic':
                              _requestThreadTopic(thread);
                            case 'edit-topic':
                              unawaited(_editActiveThreadTopic(context));
                            case 'open-reference':
                              if (referencedThread != null) {
                                _openThread(referencedThread);
                              }
                            case 'remove-reference':
                              unawaited(
                                widget.onRequest({
                                  'action': 'clear_related_thread',
                                  if (thread.channelId != null)
                        ...
                        itemBuilder: (context) => [
                          const PopupMenuItem(
                            ...
                            child: ListTile(
                              ...
                              title: Text('Request thread topic'),
                            ),
                          ),
                          const PopupMenuItem(
                            value: 'edit-topic',
                          ...
                          if (referencedThread != null) ...[
                            ...
                            PopupMenuItem(
                              ...
                              child: ListTile(
                                ...
                                title: Text(
                                  'Referencing ${_threadTopicFor(referencedThread) ?? 'thread'}',
                                ),
                              ),
                            ),
                            const PopupMenuItem(
  ...
  Map<String, String> _conversationActivityLabels() {
    final cached = _cachedConversationActivityLabels;
    ...
    return _cachedConversationActivityLabels = labels;
  }
  ...
  void _scheduleTypingExpiry(Iterable<WorkspaceTyping> typing) {
    _typingExpiryTimer?.cancel();
    final expiries = typing
        .map((status) => status.expiresAt)
        .toList(growable: false);
    if (expiries.isEmpty) return;
    final nextExpiry = expiries.reduce((a, b) => a < b ? a : b);
...
class _ConversationAgentsPage extends StatelessWidget {
  ...
  List<String> get _conversationFolderScope =>
      workspace.conversationFolderScope(
        channelId: channelId,
        ownPubkey: ownPubkey,
        peerPubkey: peerPubkey,
      );

  Future<void> _editConversationFolders(BuildContext context) async {
    final folders = await showDialog<List<String>>(
      context: context,
      builder: (_) => _FolderScopeDialog(
        initialSelected: _conversationFolderScope,
  ...
  Future<void> _editConversationBrief(BuildContext context) async {
    final controller = TextEditingController(
      text: workspace.conversationPreprompt(
        channelId: channelId,
        ownPubkey: ownPubkey,
        peerPubkey: peerPubkey,
      ),
  ...
  Future<void> _rename(BuildContext context, WorkspaceAgent agent) async {
        ...
        return ListView(
          ...
          children: [
            ...
            ListTile(
              ...
              title: const Text('Conversation directories'),
              subtitle: Text(
                _conversationFolderScope.isEmpty
                    ? 'Set a shared working directory and references'
                    : 'Working: ${_conversationFolderScope.first}${_conversationFolderScope.length > 1 ? ' · ${_conversationFolderScope.length - 1} reference' : ''}',
              ),
              trailing: const Icon(Icons.edit_outlined),
              onTap: () => _editConversationFolders(context),
            ),
            ListTile(
              contentPadding: EdgeInsets.zero,
          ...
          Row(
            children: [
              ...
              IconButton(
                ...
                onPressed: () => _rename(context, agent),
...
class _FolderScopeDialogState extends State<_FolderScopeDialog> {
  late final _selected = List<String>.of(widget.initialSelected);

  Future<void> _chooseFolder({bool reference = false}) async {
    final selection = await Navigator.of(context).push<_WorkingFolderSelection>(
      MaterialPageRoute(
        builder: (_) => _WorkingFolderPickerPage(
          ...
          selectedPath: reference ? '' : _selected.firstOrNull ?? '',
          onLoadFolders: widget.onLoadFolders,
        ),
      ),
    );
  ...
  Widget build(BuildContext context) => AlertDialog(
    ...
    content: SizedBox(
      ...
      child: Column(
        ...
        children: [
          const Text(
            'The first directory is used for work. Additional directories are read-only reference context for every agent in this conversation.',
          ),
          const SizedBox(height: 12),
          ListTile(
            contentPadding: EdgeInsets.zero,
            leading: const Icon(Icons.folder_outlined),
            title: Text(_selected.firstOrNull ?? 'Choose working directory'),
            subtitle: const Text('Working directory'),
            trailing: const Icon(Icons.chevron_right),
            onTap: _chooseFolder,
          ),
          for (final folder in _selected.skip(1))
            ListTile(
              contentPadding: EdgeInsets.zero,
              leading: const Icon(Icons.visibility_outlined),
              title: Text(folder),
              subtitle: const Text('Read-only reference directory'),
              trailing: IconButton(
                tooltip: 'Remove reference directory',
                icon: const Icon(Icons.close),
                onPressed: () => setState(() => _selected.remove(folder)),
              ),
            ),
          TextButton.icon(
            onPressed: () => _chooseFolder(reference: true),
            icon: const Icon(Icons.add),
            label: const Text('Add reference directory'),
          ),
        ],
      ),
    ),
    actions: [
      ...
      FilledButton(
        ...
        child: const Text('Save directories'),
      ),
    ],
  );
}
...
class _WorkspaceThreadsViewState extends State<_WorkspaceThreadsView> {
  ...
  final _selectedTopicWords = <String>{};
  ...
  @override
  void didUpdateWidget(covariant _WorkspaceThreadsView oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.selectedTopic != widget.selectedTopic) _applySelectedTopic();
  }

  void _applySelectedTopic() {
    ...
    _selectedTopicWords
      ...
      ..addAll(
        topic == null
            ? const []
            : _topicWords(topic).map((word) => word.toLowerCase()),
      );
    _searchQuery = topic == null ? '' : '#$topic';
  ...
  String _conversationLabel(WorkspaceMessage source) {
    if (source.channelId case final channelId?) {
  ...
  int _lastActivityAt(WorkspaceMessage source) {
    final messages =
        widget.workspace.messages[widget.workspace.conversationKeyForMessage(
          source,
        )] ??
        const <WorkspaceMessage>[];
    var lastActivity = source.createdAt;
    for (final message in messages) {
      if (message.parentId == source.id && message.createdAt > lastActivity) {
  ...
  String? _topicFor(WorkspaceMessage source) {
    final conversationKey = widget.workspace.conversationKeyForMessage(source);
    return workspaceThreadTopic(
      (widget.workspace.messages[conversationKey] ?? const <WorkspaceMessage>[])
          .where((message) => message.parentId == source.id),
    );
  }

  List<String> _topicsFor(WorkspaceMessage source) {
    final conversationKey = widget.workspace.conversationKeyForMessage(source);
    final messages = [
      source,
    ...
    void add(String topic) {
      if (seen.add(topic.toLowerCase())) topics.add(topic);
    }

    if (_topicFor(source) case final topic?) add(topic);
    for (final message in messages) {
      for (final hashtag in workspaceMessageHashtags(message.body)) {
        add(hashtag);
      }
    }
    return topics;
  ...
  Iterable<String> _topicWords(String topic) =>
      RegExp(r'[A-Za-z0-9]+').allMatches(topic).map((match) => match.group(0)!);

  bool _matchesSelectedTopicWords(WorkspaceMessage source) {
    if (_selectedTopicWords.isEmpty) return true;
    final words = _topicsFor(
      source,
    ).expand(_topicWords).map((word) => word.toLowerCase()).toSet();
    return _selectedTopicWords.every(words.contains);
  }

  bool _matchesSenderFilter(WorkspaceMessage source) => switch (_senderFilter) {
    'mine' => source.senderPubkey == widget.ownPubkey,
    'agents' => isWorkspaceAgentSender(source.senderPubkey),
    _ => true,
  };

  bool _isTopicPhraseSelected(String topic) => _topicWords(
    topic,
  ).map((word) => word.toLowerCase()).every(_selectedTopicWords.contains);

  void _setTopicPhraseSelected(String topic, bool selected) {
    final words = _topicWords(topic).map((word) => word.toLowerCase());
    setState(() {
      if (selected) {
        _selectedTopicWords.addAll(words);
        _searchQuery = '#$topic';
        _searchController.value = TextEditingValue(
          text: _searchQuery,
          selection: TextSelection.collapsed(offset: _searchQuery.length),
        ...
        _selectedTopicWords.removeAll(words);
        if (_searchQuery.toLowerCase() == '#${topic.toLowerCase()}') {
          _searchQuery = '';
  ...
  Future<void> _showTopicPicker(
    List<({String label, int count})> topics,
  ) async {
    final selected = await showModalBottomSheet<String>(
      context: context,
      showDragHandle: true,
      isScrollControlled: true,
      builder: (context) => _ThreadTopicPicker(topics: topics),
    );
    if (!mounted || selected == null) return;
    if (selected.isEmpty) {
      setState(() {
        _selectedTopicWords.clear();
        _searchQuery = '';
        _searchController.clear();
      });
    } else {
      _setTopicPhraseSelected(selected, true);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    for (final messages in widget.workspace.messages.values) {
      ...
      for (final message in messages) {
        ...
        final sentByLocalMember = message.senderPubkey == widget.ownPubkey;
        if (message.parentId == null && !isMentioned && !sentByLocalMember) {
          continue;
        }
    ...
    final topicsByPhrase = <String, ({String label, int count})>{};
    for (final source in threads.values) {
      for (final topic in _topicsFor(source)) {
        final key = topic.toLowerCase();
        final existing = topicsByPhrase[key];
        topicsByPhrase[key] = (
    ...
    final topics = topicsByPhrase.values.toList()
      ..sort(
        (left, right) =>
            left.label.toLowerCase().compareTo(right.label.toLowerCase()),
      );
    final recentTopics = <String>[];
    final seenRecentTopics = <String>{};
    final threadsByActivity = threads.values.toList()
      ..sort(
        (left, right) =>
            _lastActivityAt(right).compareTo(_lastActivityAt(left)),
      );
    for (final source in threadsByActivity) {
      for (final topic in _topicsFor(source)) {
        if (seenRecentTopics.add(topic.toLowerCase())) recentTopics.add(topic);
      }
    }
    final selectedTopic = topics
    ...
    final ordered =
        threads.values
            ...
            .where(_matchesSelectedTopicWords)
            .where(_matchesSenderFilter)
            .where(
              (source) =>
                  !_showUnreadOnly ||
                  (widget.threadUnreadCounts['${widget.workspace.conversationKeyForMessage(source)}:${source.id}'] ??
                          0) >
                      0,
            )
            .toList()
    ...
    final conversationsWithDraftShown = <String>{};
    for (final source in ordered) {
      final conversationKey = widget.workspace.conversationKeyForMessage(
        source,
      );
      final draft = widget.conversationDrafts[conversationKey];
      if (draft?.value.text.trim().isNotEmpty == true &&
          conversationsWithDraftShown.add(conversationKey)) {
        conversationDraftThreadKeys.add('$conversationKey:${source.id}');
      }
    }
    return ColoredBox(
      ...
      child: ListView(
        ...
        children: [
          ...
          TextField(
            ...
            decoration: InputDecoration(
              isDense: true,
              hintText: 'Filter by topic or conversation...',
          ...
          Wrap(
            ...
            children: [
              for (final filter in const [
                ...
                ChoiceChip(
                  label: Text(filter.$2),
                  selected: filter.$1 == 'unread'
                      ? _showUnreadOnly
                      : _senderFilter == filter.$1 && !_showUnreadOnly,
                  onSelected: (_) => setState(() {
                    if (filter.$1 == 'unread') {
                      ...
                      _senderFilter = filter.$1;
                    }
                  }),
                ),
              _ThreadTopicPickerButton(
                label: 'Topic: ${selectedTopic ?? 'Any topic'}',
                tooltip: 'Filter by topic',
                onPressed: () => _showTopicPicker(topics),
              ),
              _ThreadToolbarMenu(
                label: _sortMostRecent ? 'Most recent' : 'Oldest first',
                tooltip: 'Sort threads',
                onSelected: (value) =>
                    setState(() => _sortMostRecent = value == 'recent'),
                items: const [
                  PopupMenuItem(value: 'recent', child: Text('Most recent')),
                  PopupMenuItem(value: 'oldest', child: Text('Oldest first')),
                ],
              ),
              if (widget.threadUnreadCounts.values.any((count) => count > 0))
                IconButton(
          ...
          if (recentTopics.isNotEmpty) ...[
            ...
            Wrap(
              ...
              children: [
                ...
                for (final topic in recentTopics.take(3))
                  FilterChip(
                    label: Text(topic),
                    selected: _isTopicPhraseSelected(topic),
                    onSelected: (selected) =>
                        _setTopicPhraseSelected(topic, selected),
                  ),
                if (recentTopics.length > 3)
                  _ThreadToolbarMenu(
                    label: 'More',
                    tooltip: 'More recent topics',
                    onSelected: (topic) => _setTopicPhraseSelected(topic, true),
                    items: [
                      for (final topic in recentTopics.skip(3))
          ...
          for (final source in ordered) ...[
            Builder(
              builder: (context) {
                ...
                final firstPreviewReplies = replies.length >= 4
                    ? replies.take(2)
                    : replies;
                final lastPreviewReplies = replies.length >= 4
                    ? replies.skip(replies.length - 2)
                    : const Iterable<WorkspaceMessage>.empty();
                final messageDraft =
                    conversationDraftThreadKeys.contains(
                      '$conversationKey:${source.id}',
                    )
                    ? widget.conversationDrafts[conversationKey]
                    : null;
                final replyDraft =
                    widget.threadDrafts['$conversationKey:${source.id}'];
                final unreadCount =
                    widget
                      ...
                      child: Ink(
                        ...
                        child: Column(
                          ...
                          children: [
                              ...
                              children: [
                                ...
                                Column(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      'Updated ${_timeLabel(_lastActivityAt(source))}',
                                      style: Theme.of(context)
                                          .textTheme
                                          .labelSmall
                                          ?.copyWith(
                            ...
                            for (final message
                                in firstPreviewReplies.indexed) ...[
                              const SizedBox(height: 8),
                              _ThreadPreviewMessage(
                            ...
                            if (hiddenMessages > 0) ...[
                              const SizedBox(height: 8),
                              _WorkspaceDateDivider(
                                label:
                                    '$hiddenMessages more ${hiddenMessages == 1 ? 'message' : 'messages'}',
                              ),
                            ],
                            for (final message
                                in lastPreviewReplies.indexed) ...[
                              const SizedBox(height: 8),
                              _ThreadPreviewMessage(
                                identity: message.$2.senderPubkey,
                                author: _memberLabel(message.$2.senderPubkey),
                            ...
                            if (replyDraft?.value.text.trim().isNotEmpty ==
                                true) ...[
                              const SizedBox(height: 10),
                              _draftPreview(
                                context,
  ...
  Widget _draftPreview(
    ...
    required _WorkspaceDraft draft,
  }) => Container(
  ...
  bool _matchesSearch(WorkspaceMessage source) {
    ...
    final replies =
        widget.workspace.messages[conversationKey] ??
        const <WorkspaceMessage>[];
    final topics = _topicsFor(source);
    final exactTopic = RegExp(r'^"(.+)"$').firstMatch(query)?.group(1);
    if (exactTopic != null) {
      return topics.any((topic) => topic.toLowerCase() == exactTopic);
    }
    final queryWords = _topicWords(
      query,
    ).map((word) => word.toLowerCase()).toSet();
    final topicWords = topics
        .expand(_topicWords)
        .map((word) => word.toLowerCase())
        .toSet();
    if (queryWords.isNotEmpty && queryWords.every(topicWords.contains)) {
      return true;
    }
    final conversation = _conversationLabel(source);
    return [
      conversation,
...
class _ThreadToolbarMenu extends StatelessWidget {
  const _ThreadToolbarMenu({
    required this.label,
...
class _ThreadTopicPickerButton extends StatelessWidget {
  const _ThreadTopicPickerButton({
    required this.label,
    required this.tooltip,
...
class _ThreadTopicPicker extends StatefulWidget {
  const _ThreadTopicPicker({required this.topics});

  final List<({String label, int count})> topics;

  @override
  State<_ThreadTopicPicker> createState() => _ThreadTopicPickerState();
}

class _ThreadTopicPickerState extends State<_ThreadTopicPicker> {
  final _searchController = TextEditingController();
  String _query = '';
                  ...
                  children: [
                    ...
                    for (final topic in topics)
                      ListTile(
                        ...
                        onTap: () => Navigator.pop(context, topic.label),
                      ),
                    if (topics.isEmpty)
                      const Padding(
                        padding: EdgeInsets.only(top: 28),
                        child: Center(child: Text('No matching topics.')),
                      ),
                  ],
                ),
              ),
...
class _ThreadPreviewMessage extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    padding: source ? const EdgeInsets.all(8) : EdgeInsets.zero,
    decoration: source
        ? BoxDecoration(
            color: Color.lerp(
...
class _WorkspaceSidebar extends StatelessWidget {
  const _WorkspaceSidebar({
    ...
    required this.onToggleArchiveConversation,
  ...
  final void Function(_WorkspaceSection, String) onToggleArchiveConversation;
  ...
  void _showConversationActions(
    ...
    final key = section == _WorkspaceSection.direct
        ? WorkspaceState.directKey(ownPubkey, id)
        : id;
    final preference =
        conversationPreferences[key] ?? const WorkspaceConversationPreference();
    showModalBottomSheet<void>(
      context: context,
      showDragHandle: true,
      builder: (context) => SafeArea(
        child: Column(
          ...
          children: [
            ListTile(
              leading: Icon(
                preference.pinned ? Icons.push_pin : Icons.push_pin_outlined,
              ),
              title: Text(
                preference.pinned ? 'Unpin conversation' : 'Pin conversation',
              ),
              onTap: () {
            ...
            ListTile(
              ...
              title: Text(
                preference.muted ? 'Unmute conversation' : 'Mute conversation',
              ),
              onTap: () {
                Navigator.pop(context);
                onToggleMuteConversation(section, id);
  ...
  Widget build(BuildContext context) {
    ...
    final archivedMembers = sortedMembers
        ...
        .toList(growable: false);
    final isWorkspaceAdmin = workspace.memberAdmins.contains(ownPubkey);
    final recentThreads =
    ...
    int unreadCountForConversation(String conversationKey) {
      final conversationUnread = unreadCounts[conversationKey] ?? 0;
      final threadUnread = threadUnreadCounts.entries
          .where((entry) => entry.key.startsWith('$conversationKey:'))
          .fold(0, (total, entry) => total + entry.value);
      // Main messages and thread replies are tracked separately so opening a
      // thread can clear only its own unread count.
      return conversationUnread + threadUnread;
    }
        ...
        child: Container(
          ...
          child: ListTile(
            ...
            trailing: action != null && showAction
                ? SizedBox(width: 84, child: action)
                : unreadCount > 0
                ? Semantics(
    ...
    Widget conversationAction(
      ...
      final key = section == _WorkspaceSection.direct
          ? WorkspaceState.directKey(ownPubkey, id)
          : id;
      final archived = conversationPreferences[key]?.archived == true;
      return Row(
        children: [
          IconButton(
          ...
          IconButton(
            onPressed: () => onToggleArchiveConversation(section, id),
            icon: Icon(
...
class _WorkspaceConversation extends StatefulWidget {
  const _WorkspaceConversation({
    ...
    required this.threadUnreadCounts,
    required this.threadCompletionOverrides,
  ...
  final Map<String, int> threadUnreadCounts;
  final Map<String, bool> threadCompletionOverrides;
...
class _WorkspaceConversationState extends State<_WorkspaceConversation> {
  ...
  Set<String> _lastMainLiveMessageIds = {};
  ...
  void didUpdateWidget(covariant _WorkspaceConversation oldWidget) {
    ...
    _lastMainLiveMessageIds = mainLiveMessageIds;
  ...
  Set<String> get _mainLiveMessageIds => {
    for (final status in widget.typingStatuses)
      if (_showsMainLiveMessage(status))
        '${status.senderPubkey}:${status.agentId ?? ''}:${status.parentId ?? ''}',
  };
  ...
  Iterable<WorkspaceMessage> _threadRepliesFor(WorkspaceMessage thread) =>
      widget.searchMessages.where((message) => message.parentId == thread.id);
  ...
  List<String> _threadReplyAgentNames(WorkspaceMessage thread) {
    final names = <String>[];
    for (final reply in widget.searchMessages) {
      if (reply.parentId != thread.id ||
          !isWorkspaceAgentSender(reply.senderPubkey)) {
        continue;
      }
      final name = _memberLabel(reply.senderPubkey);
      if (!names.contains(name)) names.add(name);
    }
    return names;
  }
      ...
      items: [
        ...
        const PopupMenuItem(
          ...
          child: ListTile(
            ...
            title: Text('Reload last messages'),
          ),
        ),
        ...
        if (widget.section == _WorkspaceSection.channel ||
          ...
          const PopupMenuItem(
            ...
            child: ListTile(
              ...
              title: Text('Browse repository files'),
            ),
          ),
        if (widget.section == _WorkspaceSection.channel)
          PopupMenuItem(
            ...
            child: ListTile(
              ...
              contentPadding: EdgeInsets.zero,
              leading: const Icon(Icons.folder_outlined),
              title: const Text('Edit shared directories'),
              subtitle: Text(
                widget.conversationFolderScope.isEmpty
                    ? 'Current workspace directory'
                    : 'Working: ${widget.conversationFolderScope.first}${widget.conversationFolderScope.length > 1 ? ' · ${widget.conversationFolderScope.length - 1} reference' : ''}',
              ),
            ),
          ),
  ...
  Future<void> _showMobileConversationActions() async {
          ...
          children: [
            ...
            if (_sharedPinnedMessages.isNotEmpty)
              ListTile(
                ...
                onTap: () => Navigator.pop(context, 'pins'),
              ),
    ...
    switch (action) {
      ...
      case 'reload':
        ...
        break;
      case 'members':
        _showConversationMembers();
        break;
      case 'pins':
        _showPinnedMessages(
          context,
          title: 'Pins',
          messages: _sharedPinnedMessages,
        );
        break;
      case 'saved':
        _showPinnedMessages(context, title: 'Saved', messages: _savedMessages);
        break;
      case 'files':
        await widget.onOpenFiles();
        break;
                      ...
                      child: LayoutBuilder(
                        builder: (context, constraints) {
                          ...
                          return ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              if (index < mainLiveMessages.length) {
                                final status = mainLiveMessages[index];
                                return Padding(
                                        ...
                                        children: [
                                          SizedBox(
                                            ...
                                            child: _WorkspaceMessageRow(
                                              ...
                                              showAuthorWhenGrouped: true,
                                              isLocalSender:
                                                  isWorkspaceLocalSender(
                                                    m.senderPubkey,
                                                    widget.localSenderIds,
                                                  ),
                                              fipsConnected: widget
                                                  .fipsConnectedPeers
                                              ...
                                              threadReplyCount:
                                                  widget.threadReplyCounts[m
                                                      .id] ??
                                                  0,
                                              threadLastReplyAt: widget
                                                  .threadLastReplyAt[m.id],
                                              threadReplyAgentNames:
                                                  _threadReplyAgentNames(m),
                                              threadUnreadCount:
                                                  widget.threadUnreadCounts[m
                                                      .id] ??
                                                  0,
                                              threadActivityLabel: widget
                                                  .threadActivityLabels[m.id],
                                              isMessageExpanded: widget
                                                  .expandedMessageIds
                                                  .contains(m.id),
                                              onMessageExpandedChanged:
                                                  (expanded) => widget
                                                      .onMessageExpandedChanged(
                                                        m.id,
                                                        expanded,
                ...
                child: Column(
                  ...
                  children: [
                    WorkspaceComposer(
                      ...
                      routeMainToAgent: widget.canUseAgentRouting
                          ? widget.routeMainToAgent
                          : null,
                      onRouteMainToAgentChanged: widget.canUseAgentRouting
                          ? widget.onRouteMainToAgentChanged
                          : null,
                    ),
                  ],
                ),
...
class _WorkspaceDateDivider extends StatelessWidget {
  const _WorkspaceDateDivider({required this.label});
...
class _WorkspaceMessageRow extends StatefulWidget {
  const _WorkspaceMessageRow({
    ...
    this.threadReplyAgentNames = const [],
    this.threadTopic,
  ...
  final ValueChanged<bool> onMessageExpandedChanged;
  final bool isCollapsed;
  final ValueChanged<bool>? onCollapsedChanged;
  final String? threadActivityLabel;
  final List<String> threadReplyAgentNames;
...
class _WorkspaceMessageRowState extends State<_WorkspaceMessageRow>
  ...
  Widget build(BuildContext context) {
    ...
    final interactiveAuthorLabel = !isAgent || widget.onOpenMention == null
        ...
        : GestureDetector(
            onTap: () {
              ...
              widget.onOpenMention!(
                WorkspaceMention(
                  kind: 'agent',
                  id: widget.message.senderPubkey.substring('agent:'.length),
                  label: widget.authorName,
                ),
              );
            },
    ...
    Widget row(double timestampWidth, {double? width}) => SizedBox(
      width: width,
      child: Stack(
        children: [
          if (widget.isCollapsed)
    ...
    final lastReplyLabel = _lastReplyLabel();
    final mutedColor = Theme.of(context).colorScheme.onSurfaceVariant;
    final replyCountStyle = Theme.of(
      context,
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        final minimumBubbleWidth = MediaQuery.sizeOf(context).width < 720
            ? 144.0
            : 300.0;
        final availableBubbleWidth = constraints.maxWidth;
        final maxBubbleWidth = math
            .max(minimumBubbleWidth, availableBubbleWidth)
            .toDouble();
        final timestampWidth = widget.isInProgress
            ? 0.0
            : (TextPainter(
                text: TextSpan(
  ...
  String? _lastReplyLabel() {
    final timestamp = widget.threadLastReplyAt;
...
class _WorkspaceFrogAvatar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final colors = (
      ...
      foregroundColor ?? defaultColors.$2,
    );
    return CircleAvatar(
      radius: circleRadius,
...
class _WorkspaceMessageText extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (after.isNotEmpty) {
      children.add(const SizedBox(height: 8));
      children.add(_buildTextWithTables(context, after, null));
    }
    return Column(
      mainAxisSize: MainAxisSize.min,
      crossAxisAlignment: CrossAxisAlignment.start,
...
class _WorkspaceMarkdownTable {
  ...
  final List<bool> alignments;
  final List<List<String>> rows;

  static List<_WorkspaceMarkdownTable> parseAll(String text) {
    final lines = text.split('\n');
    final tables = <_WorkspaceMarkdownTable>[];
    var characterOffset = 0;
    for (var index = 0; index < lines.length - 1; index++) {
      final headers = _cells(lines[index]);
      final separators = _cells(lines[index + 1]);
      if (headers == null ||
          separators == null ||
      ...
      final start = characterOffset;
      final rows = <List<String>>[];
      var rowIndex = index + 2;
      while (rowIndex < lines.length) {
        final cells = _cells(lines[rowIndex]);
        if (cells == null) break;
        rows.add(
          List.generate(headers.length, (column) {
            return column < cells.length ? cells[column] : '';
          }),
        );
        rowIndex++;
      }
      final end = start + lines.sublist(index, rowIndex).join('\n').length;
      tables.add(
        _WorkspaceMarkdownTable(
          start: start,
          end: end,
          headers: headers,
          alignments: separators.map((cell) => cell.endsWith(':')).toList(),
          rows: rows,
        ),
      );
  ...
  static bool _isSeparator(String cell) =>
      RegExp(r'^:?-{3,}:?$').hasMatch(cell);
}

class _WorkspaceMarkdownTableView extends StatelessWidget {
  const _WorkspaceMarkdownTableView({
    required this.table,
    required this.mentions,
    required this.onOpenMessageReference,
  ...
  final _WorkspaceMarkdownTable table;
  final List<WorkspaceMention> mentions;
  final ValueChanged<String> onOpenMessageReference;
  final ValueChanged<WorkspaceMention>? onOpenMention;
  final ValueChanged<String>? onOpenTopic;
...
class _WorkspaceMentionDetails extends StatelessWidget {
  ...
  List<String> _visibleDirectories(WorkspaceAgent agent) => workspace
      ...
      .where((membership) => membership.agentId == agent.id)
      .expand(
        (membership) => workspace.conversationFolderScope(
          channelId: membership.channelId,
          ownPubkey: ownPubkey,
          peerPubkey: membership.channelId == null
...
class _WorkspaceContext extends StatelessWidget {
  const _WorkspaceContext({
    ...
    required this.onCloseAllThreads,
    required this.mentionOptions,
    required this.onMentionSelected,
    required this.onSend,
    required this.onAttach,
  ...
  final VoidCallback onCloseAllThreads;
  final List<WorkspaceMention> mentionOptions;
  final ValueChanged<WorkspaceMention> onMentionSelected;
  final VoidCallback onSend;
  final Future<void> Function() onAttach;
  ...
  Future<void> _closeAllThreads(BuildContext context) async {
    if (openThreads.length > 1) {
      final confirmed = await showDialog<bool>(
        context: context,
        builder: (context) => AlertDialog(
          title: const Text('Close all threads?'),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(context, false),
              child: const Text('Cancel'),
  ...
  Widget _threadTitleSelector(BuildContext context) {
    final title = _threadTitle(topicOverride.value);
    final hasUnreadOtherThread = openThreads.any(
      (thread) =>
  ...
  String _threadTitleFor(WorkspaceMessage thread) {
    return threadTitles[thread.id] ?? 'Thread';
  }
  ...
  List<WorkspaceMessage> _repliesFor(WorkspaceMessage thread) =>
      conversationMessages
          .where((candidate) => candidate.parentId == thread.id)
          .toList(growable: false);

  WorkspaceMessage? get _referencedThread {
    final current = message;
    if (current == null) return null;
    final roots = {
      for (final candidate in conversationMessages) candidate.id: candidate,
    };
    return roots[workspaceActiveRelatedThreadId(conversationMessages, current)];
  }

  List<WorkspaceMessage> get _relatedThreadBacklinks {
    final current = message;
    return current == null
        ? const []
        : workspaceRelatedThreadBacklinks(conversationMessages, current);
  }

  List<WorkspaceMessage> get _replacementThreadCandidates {
    final current = message;
    return current == null
        ? const []
        : workspaceRelatedThreadCandidates(
  ...
  String _threadPreview(WorkspaceMessage thread) {
    final messages = [thread, ..._repliesFor(thread)]
        .where(
          (candidate) =>
              !isWorkspaceThreadTopicControlMessage(candidate) &&
              !isWorkspaceRelatedThreadControlMessage(candidate),
        )
        .toList(growable: false);
    final excerpt = messages.length <= 4
        ? messages
        : [...messages.take(2), ...messages.skip(messages.length - 2)];
    return excerpt
        .map((candidate) => workspaceDisplayMessageText(candidate.body).trim())
        .where((body) => body.isNotEmpty)
        .join('\n');
  }

  Future<void> _offerRelatedThread(
    BuildContext context, {
    bool replace = false,
  }) async {
    final current = message;
    final candidates = replace
        ? _replacementThreadCandidates
        : relatedThreadCandidates;
    if (current == null || candidates.isEmpty) return;
    final related = candidates.first;
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: Text(
          '${replace ? 'Replace with' : 'Continue'} ${_threadTitleFor(related)}?',
        ),
        content: Text(
          'The earlier thread\'s agent will continue this work when available.\n\n${_threadPreview(related)}\n\n'
          'The current thread stays separate and new replies remain here.',
        ),
        actions: [
          TextButton(
            onPressed: () {
              Navigator.pop(context);
              onSelectThread(related);
            },
            child: const Text('Open prior thread'),
          ),
          TextButton(
            onPressed: () => Navigator.pop(context, false),
    ...
    if (confirmed != true) return;
    await onRequest({
      'action': replace ? 'replace_related_thread' : 'link_related_thread',
    ...
    await onRequest({
      'action': 'reactivate_thread_agent',
      if (current.channelId != null) 'channel_id': current.channelId,
      if (current.recipientPubkey != null)
        'recipient_pubkey': current.recipientPubkey,
      'parent_id': current.id,
  ...
  Future<void> _removeRelatedThread() async {
    final current = message;
    if (current == null || _referencedThread == null) return;
    await onRequest({
      'action': 'clear_related_thread',
      if (current.channelId != null) 'channel_id': current.channelId,
      if (current.recipientPubkey != null)
  ...
  Widget _messageRow(
    ...
    message: message,
    authorName: isWorkspaceLocalSender(message.senderPubkey, localSenderIds)
        ? 'You'
        : _memberLabel(message.senderPubkey),
                    ...
                    children: [
                      Expanded(
                        child: _ThreadTopicActions(
                          ...
                          onEditTopic: () =>
                              unawaited(_editThreadTopic(context)),
                          child: ValueListenableBuilder<String?>(
                            valueListenable: topicOverride,
                            builder: (context, _, _) =>
                                _threadTitleSelector(context),
                          ),
                        ),
                      ),
                      IconButton(
                      ...
                      IconButton(
                        tooltip: 'Close this thread',
                        onPressed: onCloseCurrentThread,
                        icon: const Icon(Icons.close, size: 18),
                      ...
                      IconButton(
                        tooltip: 'Close thread panel',
                        onPressed: () => unawaited(_closeAllThreads(context)),
                        icon: const Icon(Icons.close),
                      ),
                    ],
                  ),
                ...
                if (!compactHeader && _referencedThread != null)
                  Padding(
                    ...
                    child: Wrap(
                      spacing: 4,
                      children: [
                        if (_referencedThread case final thread?)
                          TextButton.icon(
                            onPressed: () => onSelectThread(thread),
                            icon: const Icon(Icons.link_outlined, size: 16),
                            label: Text(
                              'Referencing ${_threadTitleFor(thread)}',
                            ),
                          ),
                        if (_referencedThread != null)
                          TextButton.icon(
                            onPressed: () => unawaited(_removeRelatedThread()),
                            icon: const Icon(Icons.link_off_outlined, size: 16),
                            label: const Text('Remove reference'),
                          ),
                        if (_replacementThreadCandidates.isNotEmpty)
                          TextButton.icon(
                            onPressed: () => unawaited(
                              _offerRelatedThread(context, replace: true),
                            ),
                            icon: const Icon(
                              Icons.swap_horiz_outlined,
                              size: 16,
                            ),
                            label: const Text('Change reference'),
                          ),
                      ],
                    ),
                  ),
                if (_relatedThreadBacklinks.isNotEmpty)
                  Padding(
                    padding: const EdgeInsets.only(bottom: 8),
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        ...
                        Wrap(
                          spacing: 4,
                          children: [
                            for (final thread in _relatedThreadBacklinks)
                              TextButton.icon(
                                onPressed: () => onSelectThread(thread),
                                icon: const Icon(Icons.link_outlined, size: 16),
                                label: Text(
                                  'Related ${_threadTitleFor(thread)}',
                                ),
                              ),
                          ],
                        ),
...
class _ThreadSearchController extends ChangeNotifier {
  final controller = TextEditingController();
  ...
  void setResultCount(int count) {
    final nextIndex = count == 0
        ? 0
        : selectedIndex.clamp(0, count - 1).toInt();
    if (resultCount == count && selectedIndex == nextIndex) return;
    resultCount = count;
                  ...
                  Row(
                    children: [
                      ...
                      IconButton(
                        ...
                        onPressed: search.resultCount == 0
                            ? null
                            : search.previous,
                      ...
                      IconButton(
                        tooltip: 'Next match',
                        onPressed: search.resultCount == 0 ? null : search.next,
                        icon: const Icon(Icons.keyboard_arrow_down),
                      ),
                      FilterChip(
                        label: const Text('Highlight all'),
                        selected: search.highlightAll,
                        onSelected: (_) => search.toggleHighlightAll(),
                        visualDensity: VisualDensity.compact,
                      ),
                    ],
                  ),
...
class _ThreadTitleDropdown extends StatefulWidget {
  const _ThreadTitleDropdown({
    required this.title,
  ...
  final String title;
  final List<WorkspaceMessage> threads;
...
class _ThreadTitleDropdownState extends State<_ThreadTitleDropdown>
    with SingleTickerProviderStateMixin {
  late final AnimationController _controller = AnimationController(
    vsync: this,
    duration: const Duration(milliseconds: 900),
  ...
  void _cycleThread(PointerScrollEvent event) {
    if (widget.threads.length < 2 || event.scrollDelta.dy == 0) return;
    final currentIndex = widget.threads.indexWhere(
      (thread) => thread.id == widget.selectedThreadId,
    );
    ...
    widget.onSelected(
      widget.threads[nextIndex < 0
          ? nextIndex + widget.threads.length
          : nextIndex],
    );
  }
    ...
    builder: (context, constraints) {
      ...
      return Listener(
        onPointerSignal: (event) {
          if (event case PointerScrollEvent()) _cycleThread(event);
        },
        child: PopupMenuButton<String>(
          tooltip: 'Switch thread',
          onSelected: (id) {
            final thread = widget.threads
                .where((thread) => thread.id == id)
                .firstOrNull;
            if (thread != null) widget.onSelected(thread);
          },
          itemBuilder: (context) => [
...
class _PulsingThreadLinkIcon extends StatefulWidget {
  const _PulsingThreadLinkIcon();

  @override
  State<_PulsingThreadLinkIcon> createState() => _PulsingThreadLinkIconState();
}

class _PulsingThreadLinkIconState extends State<_PulsingThreadLinkIcon>
    with SingleTickerProviderStateMixin {
  late final AnimationController _controller = AnimationController(
    vsync: this,
    duration: const Duration(milliseconds: 1200),
...
class _ThreadMessageListState extends State<_ThreadMessageList> {
  ...
  void _revealSearchResult(String messageId) {
    setState(() => _flashReplyId = messageId);
    Future<void>.delayed(const Duration(milliseconds: 1800), () {
      if (mounted && _flashReplyId == messageId) {
        setState(() => _flashReplyId = null);
      }
    });
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      animation: widget.threadSearch,
      builder: (context, _) => _buildMessages(context),
    );
  }
  ...
  Widget _buildMessages(BuildContext context) {
    ...
    final matchingMessages = query.isEmpty
        ...
        : messages
              ...
              .toList(growable: false);
    widget.threadSearch.setResultCount(matchingMessages.length);
    final selectedSearchMessage = matchingMessages.isEmpty
        ? null
        : matchingMessages[widget.threadSearch.selectedIndex];
    if (selectedSearchMessage?.id != _lastSearchResultId) {
      _lastSearchResultId = selectedSearchMessage?.id;
      if (selectedSearchMessage != null) {
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (mounted && _lastSearchResultId == selectedSearchMessage.id) {
            _revealSearchResult(selectedSearchMessage.id);
          }
        });
      }
    }
    ...
    String searchQueryFor(WorkspaceMessage message) {
      ...
      return widget.threadSearch.highlightAll ||
              message.id == selectedSearchMessage?.id
          ? query
          : '';
    }
...
class _WorkspaceMentionOverlayState extends State<_WorkspaceMentionOverlay> {
  ...
  void _syncOverlay() {
    if (widget.mentionOptions.isEmpty) {
      if (_overlayShown) _overlayController.hide();
      _overlayShown = false;
    } else {
  ...
  Widget build(BuildContext context) {
    ...
    final visibleResultCount = widget.mentionOptions.length > 8
        ? 8
        : widget.mentionOptions.length;
    final height = visibleResultCount * rowHeight;
    return LayoutBuilder(
      builder: (context, constraints) {
...
class WorkspaceComposer extends StatelessWidget {
  const WorkspaceComposer({
    ...
    this.alsoSendToMain,
    this.onAlsoSendToMainChanged,
    this.routeMainToAgent,
    this.onRouteMainToAgentChanged,
    this.canCompleteThread = false,
    this.threadCompleted = false,
    this.onCompleteThread,
    this.sendToConversationLabel,
  });
  ...
  final bool? routeMainToAgent;
  final ValueChanged<bool>? onRouteMainToAgentChanged;
  final bool canCompleteThread;
  final bool threadCompleted;
  final Future<void> Function()? onCompleteThread;
  final String? sendToConversationLabel;
  ...
  bool _hasAgentMention(String text) {
    final mentionController = composer is _WorkspaceMentionTextEditingController
        ? composer as _WorkspaceMentionTextEditingController
        : null;
    return mentionController?.mentions.any(
          (mention) =>
              mention.kind == 'agent' &&
              RegExp(
                '(?<!\\w)@${RegExp.escape(mention.label)}(?![\\w-])',
                caseSensitive: false,
              ).hasMatch(text),
        ) ??
        false;
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: voiceRecording
                        ...
                        : hintText,
                    filled: true,
                    fillColor: const Color(0xff142124),
                    ...
                    suffixIcon: Row(
                      ...
                      children: [
                        ...
                        if (alsoSendToMain case final enabled?
                            when !_hasAgentMention(value.text)) ...[
                          if (desktop && value.text.trim().isEmpty)
                            Semantics(
                              button: true,
                              toggled: enabled,
                              label: 'Also send to $sendToConversationLabel',
                              child: GestureDetector(
                                behavior: HitTestBehavior.opaque,
                                onTap: () =>
                                    onAlsoSendToMainChanged?.call(!enabled),
                                child: ConstrainedBox(
                                  constraints: const BoxConstraints(
                                    maxWidth: 120,
                                  ),
                                  child: Text(
                                    'Also send to $sendToConversationLabel',
                                    maxLines: 1,
                                    overflow: TextOverflow.ellipsis,
                                    style: Theme.of(context)
                                        .textTheme
                          ...
                          IconButton(
                            tooltip: enabled
                                ? 'Also sending to $sendToConversationLabel'
                                : 'Also send to $sendToConversationLabel',
                            onPressed: () =>
                                onAlsoSendToMainChanged?.call(!enabled),
                            icon: Icon(
      ...
      children: [
        ...
        for (final membership in memberships)
          ListTile(
            ...
            subtitle: const Text(
              'Directories are configured in this conversation.',
            ),
          ),
        const SizedBox(height: 20),
    ...
    content: SingleChildScrollView(
      child: Column(
        ...
        children: [
          ...
          if (!widget.conversationScoped)
            DropdownButtonFormField<String>(
              initialValue: _workingFolder ?? '',
              isExpanded: true,
              decoration: const InputDecoration(
                labelText: 'Working directory',
                helperText: 'Choose where this agent can read and write.',
                prefixIcon: Icon(Icons.folder_outlined),
              ),
              items: [
                const DropdownMenuItem(
                  value: '',
                  child: Text('Worker default'),
                ),
...
class _PeopleDirectoryState extends State<_PeopleDirectory> {
  ...
  bool _hasSharedConversation(String pubkey) =>
      pubkey == widget.ownPubkey ||
      widget.workspace.directPeers(widget.ownPubkey).contains(pubkey) ||
      widget.workspace.channels.any(
        (channel) =>
            channel.members.any(
              (member) => member.pubkey == widget.ownPubkey,
            ) &&
            channel.members.any((member) => member.pubkey == pubkey),
      );
    ...
    return ListView(
      ...
      children: [
        ...
        for (final person in people)
          Builder(
            builder: (context) {
              final newMember = !_hasSharedConversation(person);
              ...
              return ListTile(
                ...
                leading: _WorkspaceFrogAvatar(
                  ...
                  backgroundColor: newMember ? teal : null,
                  foregroundColor: newMember ? const Color(0xff134e4a) : null,
                ),
                title: Text(
                  _memberLabel(person),
                  style: newMember ? const TextStyle(color: teal) : null,
                ),
                subtitle: Text(
      ...
      Row(
        children: [
          TweenAnimationBuilder<double>(
            tween: Tween(begin: 0.65, end: connected ? 1 : 0.8),
...
List<_DiagnosticEventData> _groupDiagnosticEvents(
  ...
  for (final entry in entries.reversed.take(80)) {
    ...
    if (event.category == _DiagnosticCategory.snapshot &&
      ...
      if (previous.title == 'Workspace synchronization' &&
          event.title == 'Workspace synchronization') {
        final count = previous.count + event.count;
        grouped.add(
          previous.copyWith(
            detail:
                'Recovered $count FIPS update gaps with workspace snapshots.',
            count: count,
          ),
        );
        continue;
...
_DiagnosticEventData _diagnosticEvent(String entry) {
  ...
  final gapSync = RegExp(
    r'^FIPS workspace update gap detected; synchronizing workspace(?: \((\d+) times\))?$',
  ).firstMatch(raw);
  if (gapSync != null) {
    final count = int.tryParse(gapSync.group(1) ?? '') ?? 1;
    return _DiagnosticEventData(
      ...
      detail: count > 1
          ? 'Recovered $count FIPS update gaps with workspace snapshots.'
          : 'Recovered a gap in FIPS updates with a workspace snapshot.',
      count: count,
    );
  }
...
String _formatFipsDuration(Duration duration) {
  ...
  if (duration.inHours > 0) {
    return minutes.remainder(60) == 0
        ? '${duration.inHours}h'
        : '${duration.inHours}h ${minutes.remainder(60)}m';
  }
  return minutes == 0
      ? '${seconds}s'
...
class _WorkerConsolePage extends StatefulWidget {
  const _WorkerConsolePage({
    ...
    required this.workspace,
    required this.workspaceRevision,
    required this.onWorkspaceRequest,
...
class _WorkerConsolePageState extends State<_WorkerConsolePage> {
  ...
  Widget build(BuildContext context) {
    ...
    final codexStatus = _map('codex_status');
    ...
    final history = (data['history'] as List? ?? const [])
        ...
        .map((entry) => entry.cast<String, dynamic>())
        .toList();
    final codexUsageHistory = history.where((sample) {
      final usage = (sample['codex_usage'] as Map?)?.cast<String, dynamic>();
      return usage?['limits'] is List && (usage!['limits'] as List).isNotEmpty;
    }).toList();
    final codexPrimaryValues = codexUsageHistory.map((sample) {
      final limits = ((sample['codex_usage'] as Map)['limits'] as List)
          .whereType<Map>()
          .toList();
      return _number(limits.first['remaining_percent']);
    }).toList();
    final hasCodexSecondaryLimit = codexUsageHistory.every((sample) {
      final limits = (sample['codex_usage'] as Map)['limits'] as List;
      return limits.whereType<Map>().length > 1;
    });
    final codexSecondaryValues = hasCodexSecondaryLimit
        ? codexUsageHistory.map((sample) {
            final limits = ((sample['codex_usage'] as Map)['limits'] as List)
                .whereType<Map>()
                .toList();
            return _number(limits[1]['remaining_percent']);
          }).toList()
        : null;
    String codexLimitLabel(int index) {
      final limits = codexUsageHistory.isEmpty
          ? const <Map>[]
          : ((codexUsageHistory.last['codex_usage'] as Map)['limits'] as List)
                .whereType<Map>()
                .toList();
      final label = index < limits.length
          ? limits[index]['label']?.toString()
          : null;
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      children: [
                        ...
                        if (codexStatus.isNotEmpty) ...[
                          const SizedBox(height: 16),
                          _ConsolePanel(
                            title: 'Codex usage',
                            subtitle: 'Account limits and tokens remaining',
                            child: SelectableText(
                              codexStatus['text']
                                          ?.toString()
                                          .trim()
                                          .isNotEmpty ==
                                      true
                                  ? codexStatus['text'].toString()
                                  : codexStatus['error']
                                            ?.toString()
                                            .trim()
                                            .isNotEmpty ==
                                        true
                                  ? 'Could not read Codex status: ${codexStatus['error']}'
                                  : 'Codex returned no status.',
                              style: Theme.of(context).textTheme.bodySmall,
                            ),
                          ),
                        ],
                        if (codexPrimaryValues.isNotEmpty) ...[
                          const SizedBox(height: 16),
                          _ConsolePanel(
                            title: 'Codex tokens remaining',
                            subtitle:
                                'Last ${codexUsageHistory.length} samples',
                            child: _HistoryLineChart(
                              values: codexPrimaryValues,
                              secondaryValues: codexSecondaryValues,
                              color: Theme.of(context).colorScheme.primary,
                              secondaryColor: Theme.of(
                                context,
                              ).colorScheme.secondary,
                              primaryLabel: codexLimitLabel(0),
                              secondaryLabel: hasCodexSecondaryLimit
                                  ? codexLimitLabel(1)
                                  : null,
                            ),
                          ),
...
class _WorkingFolderPickerPageState extends State<_WorkingFolderPickerPage> {
  ...
  void initState() {
    ...
    // Folder lists are scoped to a worker. Do not show a cached list from a
    // different workspace while this picker is loading the active worker root.
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          _TextScaleSettings(scale: textScale, onChanged: onTextScaleChanged),
          const SizedBox(height: 16),
          Card(
            child: ListTile(
...
class _TextScaleSettings extends StatelessWidget {
  const _TextScaleSettings({required this.scale, required this.onChanged});
